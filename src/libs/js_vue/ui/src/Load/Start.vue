    ...
    <div>
        ...
        <div id="load" v-if="show">
            ...
            <div v-if="model3.show" class="model3">
                ...
                <img :src="model3.img" @error="$bb.imgError()" />
            </div>
        </div>
    </div>
</template>
<script>
    export default {
        ...
        data() {
            return {
                ...
                model3: {//引导广告
                    ...
                    timeF(this_) {
                        this_.show = 1;
                        let ipcode = this_.$bb_link.get('ipcode');
                ...
                typeF()
                {
                    switch (this.load_model) {
                        case 1://进度条
                            ...
                            this.model2.show = 0;
                            this.model3.show = 0;
                            this.$bb_data.request(this.model1.request);
                            this.$bb_data.response(this.model1.response);
                            break;
                        case 2://转圈圈
                            this.model1.show = 0;
                            this.model2.show = 1;
                            this.model3.show = 0;
                            this.$bb_data.request(this.model2.request);
                            this.$bb_data.response(this.model2.response);
                            setTimeout(()=>{
        ...
        created() {
            this.load_model = this.$bb_data.load.model;
            if(this.$route.name==='Home'){
                if(!this.is_route&&!this.$bb_data.getLocal_('reload_is')){
                    this.model1.show = 0;
                    this.model2.show = 0;
                    this.model3.show = 1;
                    this.model3.timeF(this);
